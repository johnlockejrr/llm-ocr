...
class TogetherOCRModel(BaseOCRModel):
    ...
    def process_full_page(self, page_image_base64: str, document_id: str) -> str:
        ...
        try:
            ...
            if isinstance(result, dict) and "line" in result and result["line"].strip().startswith('['):
                try:
            ...
            if isinstance(result, dict) and "line" in result:
                return result["line"]
            else:
    ...
    def correct_text_with_paragraphs(self, text: str, image_base64: str) -> Union[str, List[str]]:
        ...
        try:
            # Prepare the image
            image_data = self._prepare_image_data(image_base64)
            
            # Send the request
            response = self._make_api_request(prompt, image_data)
