...
from llm_ocr.models import ProcessingMode, Line
...
class OCRPipelineWorkflow:
    ...
    def _get_image_str(self):
        """Convert image to base64 string."""
        with open(self.image_path, "rb") as image_file:
            image_str = base64.b64encode(image_file.read()).decode('utf-8')
        return image_str
    
    def _add_history_entry(self, step: str, mode: str = None):
    ...
    def _save_results(self):
        ...
        # Update timestamp
        self.results["document_info"]["last_updated"] = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        for mode in self.modes:
            ...
            if not self.rerun:
                if self.results["models"][self.model_name]["ocr_results"].get(mode.value):
                    lines_existed = self.results["models"][self.model_name]["ocr_results"][mode.value].get("lines")
                    if lines_existed:
            ...
            logging.info(f"Processed {len(lines)} lines with mode {mode.value} in {completion_time:.2f} seconds")
            ...
            for model_name, model_results in results.items():
                ...
                self.results["models"][self.model_name]["ocr_results"][mode.value]["lines"] = serializable_lines
    ...
    def evaluate_ocr(self):
        ...
        for mode, mode_results in self.results["models"][self.model_name]["ocr_results"].items():
            ...
            try:
                ...
                completion_time = time.time() - start_time
                logging.info(f"Evaluation completed for mode {mode} in {completion_time:.2f} seconds")
                
                # Add to history
                self._add_history_entry(step="ocr_evaluation", mode=mode)
    ...
    def run_correction(self, correction_mode: str = "line"):
        """
        ...
        optimized_image_str, _ = resize_image_to_dpi(self.image_path, self.target_dpi, max_pixels=2000000)
        ...
        # Add to history
        self._add_history_entry(step="correction")
    ...
    def evaluate_correction(self):
        ...
        try:
            ...
            # Add to history
            self._add_history_entry(step="correction_evaluation")
...
# Utility function to run the complete pipeline for one model
def run_ocr_workflow(
    ...
    """
    Run the complete OCR pipeline for a single model.
...
# Utility function to run the pipeline with multiple models
def run_multi_model_workflow(
    xml_path: str,
    image_path: str,
    ground_truth_path: Optional[str] = None,
    model_names: List[str] = ["claude-3-7-sonnet-20250219", "gpt-4o-2024-08-06"],
    output_dir: str = "ocr_outputs",
    prompt_version: PromptVersion = PromptVersion.V1
) -> Dict:
    """
    Run the OCR pipeline with multiple models.
    ...
    Returns:
        Dictionary with all results including all models
    """
    results = None
    ...
    for model_name in model_names:
        ...
        # Each model updates the same results file
