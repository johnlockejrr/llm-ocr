...
def resize_image_to_dpi(image_path: str, target_dpi: int = 150, max_pixels: int = 2000000) -> Tuple[str, Dict[str, Any]]:
    """
    ...
    try:
        ...
        logging.info(f"Original image: {original_info['width']}×{original_info['height']} ({original_info['total_pixels']:,} pixels) at {original_info['dpi_x']}×{original_info['dpi_y']} DPI, {original_info['file_size_mb']:.2f} MB")
        ...
        with Image.open(image_path) as img:
            ...
            if not dpi_resize_needed and not pixel_resize_needed:
                logging.info(f"Image DPI ({current_dpi}) ≤ target ({target_dpi}) and pixels ({current_pixels:,}) ≤ max ({max_pixels:,}). No resizing needed.")
            ...
            pixel_scale_factor = (max_pixels / current_pixels) ** 0.5 if pixel_resize_needed else 1.0
            ...
            logging.info(f"Dimensions: {img.width}×{img.height} → {new_width}×{new_height} (scale: {scale_factor:.3f})")
            ...
            if save_format == 'JPEG':
                ...
                resized_img.save(buffer, format=save_format, quality=95, dpi=(target_dpi, target_dpi))
            else:
            ...
            logging.info(f"  File size: {final_info['file_size_mb']:.2f} MB ({size_reduction:.1f}% reduction)")
            logging.info(f"  Pixels: {final_info['total_pixels']:,} ({pixel_reduction:.1f}% reduction)")
...
def resize_image_for_llm(image_path: str, target_size: str = "medium") -> Tuple[str, Dict[str, Any]]:
    """
    ...
    logging.info(f"Using '{target_size}' preset: max {preset['max_pixels']:,} pixels, {preset['target_dpi']} DPI")
    
    return resize_image_to_dpi(
        image_path=image_path,
        target_dpi=preset["target_dpi"],
