...
from ..evaluators.evaluator import OCREvaluator, MetricsComparer
...
class OCRCorrectionPipeline:
    ...
    def __init__(self, model: BaseOCRModel, evaluator: OCREvaluator, config: Optional[EvaluationConfig] = None):
        """
    ...
    def _evaluate_correction(
        ...
        try:
            ...
            result.improvement = {
                'character_accuracy_delta': round(corrected_metrics.char_accuracy - ocr_metrics.char_accuracy, 10),
                'word_accuracy_delta': round(corrected_metrics.word_accuracy - ocr_metrics.word_accuracy, 10),
                'old_char_preservation_delta': round(corrected_metrics.old_char_preservation - ocr_metrics.old_char_preservation, 10),
                'case_accuracy_delta': round(corrected_metrics.case_accuracy - ocr_metrics.case_accuracy, 10),
            }
            ...
            if self.config.include_detailed_analysis:
                ...
                result.similarity = self.evaluator.calculate_similarity(ground_truth, corrected_text)
