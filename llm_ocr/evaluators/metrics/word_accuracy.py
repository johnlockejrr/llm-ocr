...
class WordAccuracyMetric(BaseMetric):
    ...
    def _align_tokens(self, ground_truth_tokens: list, extracted_tokens: list, 
                      partial_match_threshold: float) -> float:
        """
        ...
        similarity_matrix = [[0 for _ in range(m)] for _ in range(n)]
        ...
        for i in range(n):
            for j in range(m):
                if ground_truth_tokens[i] == extracted_tokens[j]:
                    ...
                    similarity = self._calculate_similarity(ground_truth_tokens[i], extracted_tokens[j])
                    similarity_matrix[i][j] = similarity if similarity >= partial_match_threshold else 0
    ...
    def evaluate(self, ground_truth: str, extracted: str, 
