...
class OverHistoricizationMetric(BaseMetric):
    ...
    def _find_insertion_contexts(self, ground_truth: str, extracted: str, 
                               context_size: int = 2) -> List[Dict[str, str]]:
        """
                ...
                if op in ('insert', 'replace'):
                    ...
                    if dest_pos < len(extracted):
                        ...
                        if inserted_char in ARCHAIC_CHARS:  # Use global constant here
                            ...
                            after = extracted[dest_pos+1:min(len(extracted), dest_pos+context_size+1)]
                            
                            # What was in the ground truth at this position?
                            original = ground_truth[src_pos] if op == 'replace' and src_pos < len(ground_truth) else "∅"
