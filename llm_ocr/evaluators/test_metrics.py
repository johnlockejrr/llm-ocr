...
from ..config import EvaluationConfig, MetricsConfig
...
class TestWordAccuracyMetric(unittest.TestCase):
    ...
    def test_partial_match(self):
        ...
        ground_truth = "This is a test."
        extracted = "This is test."
        # 3 matching words out of 4 words
        expected = 3 / 4
        self.assertAlmostEqual(self.metric.evaluate(ground_truth, extracted), expected, places=3)
...
class TestErrorAnalysisMetric(unittest.TestCase):
    ...
    def test_substitutions(self):
        ...
        result = self.metric.evaluate(ground_truth, extracted)
        # 2 substitutions: 'i'->'a' and 's'->'x'
        self.assertEqual(len(result["substitutions"]), 2)
        self.assertEqual(result["substitutions"][0]["ground_truth"], "i")
        self.assertEqual(result["substitutions"][0]["extracted"], "a")
...
class TestWordAnalysisMetric(unittest.TestCase):
    ...
    def test_word_deletions(self):
        ...
        self.assertEqual(result["word_deletions"][0]["words"], ["a", "test."])
    
    def test_word_insertions(self):
        """Test word insertions."""
        ground_truth = "This is."
        extracted = "This is a test."
        result = self.metric.evaluate(ground_truth, extracted)
        # 1 insertion: 'a test'
        self.assertEqual(len(result["word_insertions"]), 1)
        self.assertEqual(result["word_insertions"][0]["words"], ["a", "test."])
...
class TestOCREvaluator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.evaluator = OCREvaluator(config, metrics_config)
    ...
    def test_disabled_metrics(self):
        ...
        metrics_config = MetricsConfig(use_char_accuracy=False)
        evaluator = OCREvaluator(EvaluationConfig(), metrics_config)
        
        # Mock the metrics to verify they're not called
