...
from ..models import OCRResult, Line
...
from llm_ocr.evaluators.metrics.character_accuracy_without_case import CaseInsensitiveCharacterAccuracyMetric
...
class OCREvaluationService:
    ...
    def _init_specialized_metrics(self):
        ...
        self.similarity_metric = SimilarityMetric(
            self.config.char_similarity_weight,
            self.config.word_similarity_weight
        )
    ...
    def compare_texts(self, ground_truth: str, extracted_text: str, 
                     processing_time: float = 0.0) -> Tuple[OCRResult, Dict[str, Any]]:
        """
    ...
    def compare_text_lists(self, ground_truth_texts: List[str], extracted_texts: List[str],
                          processing_times: Optional[List[float]] = None) -> Dict[str, Any]:
        """
        ...
        if processing_times is None:
            ...
            self.logger.warning(f"Processing times list length ({len(processing_times)}) doesn't match text list length ({len(ground_truth_texts)})")
            processing_times = processing_times[:len(ground_truth_texts)] + [0.0] * (len(ground_truth_texts) - len(processing_times))
            
        # Create OCRResult objects for each pair
        results = []
        for i, (gt, ext) in enumerate(zip_longest(ground_truth_texts, extracted_texts, fillvalue="")):
    ...
    def compare_line_objects(self, ground_truth_lines: List[Line], extracted_texts: List[str],
                           processing_times: Optional[List[float]] = None) -> Dict[str, Any]:
        """
    ...
    def compare_line_pairs(self, line_pairs: List[Tuple[str, str]], 
                          processing_times: Optional[List[float]] = None) -> Dict[str, Any]:
        """
    ...
    def _extract_metrics_dict(self, metrics) -> Dict[str, Any]:
        ...
        metrics_dict = {
            ...
            "old_char_preservation": metrics.old_char_preservation,
            "case_accuracy": metrics.case_accuracy,
            "case_errors": metrics.case_errors,
            "case_accuracy_case_insensitive": metrics.char_accuracy_case_insensitive,
            # "historicization_errors": metrics.historicization_errors,
    ...
    def generate_report(self, results: List[OCRResult], 
    ...
    def convert_to_ocr_results(self, raw_results: List) -> List:
        """
        ...
        Returns:
            Dictionary mapping model names to lists of OCRResult objects
        """
        results = []
    ...
    def evaluate_ocr_results(self, raw_results: List, include_details: bool = False) -> List:
        """
        ...
        Returns:
            Dictionary with evaluation report data by model
        """
    ...
    def _calculate_model_metrics(self, results: List[OCRResult]) -> Dict[str, float]:
        """
        ...
        try:
            metrics = {
                ...
                "character_accuracy": np.mean([r.metrics.char_accuracy for r in results]),
                "char_accuracy_case_insensitive": np.mean([r.metrics.char_accuracy_case_insensitive for r in results]),
                "word_accuracy": np.mean([r.metrics.word_accuracy for r in results]),
                "old_char_preservation": np.mean([r.metrics.old_char_preservation for r in results]),
                "case_accuracy": np.mean([r.metrics.case_accuracy for r in results]),
                "total_lines": len(results),
                "total_case_errors": sum(len(r.metrics.case_errors) for r in results)
            }
            ...
            if historicization_results:
                ...
                for char, _ in char_frequencies.most_common():
                    contexts_for_char = [
                        ...
                        if ctx.get("char") == char
                    ][:5]  # Limit to 5 contexts per character
        ...
        try:
            for result in results:
                ...
                for sub in error_analysis["substitutions"]:
                    patterns["common_char_errors"][f"{sub['ground_truth']}->{sub['extracted']}"] += 1
                ...
                for w1, w2 in zip_longest(words_gt, words_ext):
                    if w1 and w2:
                        ...
                        if len_diff != 0:
                            patterns["word_length_errors"][len_diff] += 1
                ...
                for err in error_analysis.get("special_char_errors", []):
                    ...
                    for gt_char in gt_chars:
                        if ext_chars:
                            ...
                            patterns["old_char_error_patterns"][f"{gt_char}->∅"] += 1  # ∅ means omitted
                ...
                for ctx in historicization.get("insertion_contexts", []):
                    if "original_char" in ctx and "char" in ctx:
                        patterns["historicization_patterns"][f"{ctx['original_char']}->{ctx['char']}"] += 1
    ...
    def _analyze_length_impact(self, results: List[OCRResult]) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                f"{k}-{k+19}_chars": np.mean(v)
                for k, v in sorted(length_groups.items())
            }
        except Exception as e:
    ...
    def save_evaluation_report(self, report: Dict[str, Dict[str, Any]], 
                              output_dir: Path, prefix: str = "evaluation"):
        """
    ...
    def _log_detailed_report(self, report_data: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info("\nBasic Metrics:")
            self.logger.info(
                f"Character Accuracy: {report_data.get('character_accuracy', 0):.2%}")
            self.logger.info(
                f"Word Accuracy: {report_data.get('word_accuracy', 0):.2%}")
            self.logger.info(
                f"Old Character Preservation: {report_data.get('old_char_preservation', 0):.2%}")
            self.logger.info(
                f"Case Accuracy: {report_data.get('case_accuracy', 0):.2%}")
            self.logger.info(
                f"Total Lines Processed: {report_data.get('total_lines', 0)}")

            # Error patterns
            if 'error_analysis' in report_data and 'common_char_errors' in report_data['error_analysis']:
                self.logger.info("\nTop Error Patterns:")
                ...
                if errors:
                    for pattern, count in list(errors.items())[:5]:
                        ...
                        self.logger.info(
                            f"'{from_char}' → '{to_char}': {count} occurrences")
                else:
