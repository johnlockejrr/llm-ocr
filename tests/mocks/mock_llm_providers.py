...
class MockClaudeModel(BaseOCRModel):
    ...
    def process_sliding_window(self, images_base64: List[str]) -> Optional[Dict[str, Any]]:
        ...
        return {
            "lines": [{"line": f"Mock Claude window line {i+1}"} for i in range(len(images_base64))]
        }
...
class MockOpenAIModel(BaseOCRModel):
    ...
    def process_sliding_window(self, images_base64: List[str]) -> Optional[Dict[str, Any]]:
        ...
        self.last_request = {"type": "sliding_window", "num_images": len(images_base64)}
        return {
            "lines": [{"line": f"Mock GPT window line {i+1}"} for i in range(len(images_base64))]
        }
...
def mock_gemini_client():
    ...
    mock_response.candidates = [
        MagicMock(
            content=MagicMock(
                parts=[MagicMock(text='{"lines": [{"line": "Mock Gemini response"}]}')]
            )
        )
    ]
